...
export class Repository {
    ...
    /**
     * Gets the file content from the target branch (old version before changes).
     * This is more token-efficient than sending the new file since the diff contains all changes.
     */
    public async GetOldFileContent(fileName: string): Promise<string> {
        const targetBranch = this.GetTargetBranch();
        
        try {
            // Use git show to get the file content from the target branch
            const oldContent = await this._repository.show([`${targetBranch}:${fileName}`]);
            return oldContent;
        } catch (error) {
            // File might be new (doesn't exist in target branch)
            tl.debug(`Could not get old file content for ${fileName} from ${targetBranch}: ${error}`);
            return '';
        }
    }
